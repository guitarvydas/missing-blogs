...
def load_corpus(directory):
    ...
    docs = []
    seen_paths = set()   # resolved canonical paths — catches symlink dupes too

    for path in sorted(Path(directory).rglob('*.md')):
        # Skip files inside any generated/vendor directory
        if any(part in SKIP_DIRS for part in path.parts):
            continue

        # Deduplicate by canonical (resolved) path
        canon = path.resolve()
        ...
        docs.append({
            ...
            'lines':      raw.count('\n'),
            'words':      word_count,
            'md5':        hashlib.md5(clean.encode()).hexdigest(),
        })
    return docs
