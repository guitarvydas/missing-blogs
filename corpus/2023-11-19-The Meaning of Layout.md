...
- A possible counter-argument to much of the above, or, does this actually support much of the above?
	- code formatting and pretty-printing
	- some modern languages specify format as part of the language spec
		- sounds similar to formalization of drafting rules in Engineering
...
- visual layout has been heavily used in the past
	- Engineering school teaches drafting - courses in layout lore
	- EE schematics
...
- The idea of VPLs seems to be conflated with the notion of parsing full-blown Art, whereas, we don't blink an eye when text-only programming languages use only a small handful of words from the English language.  I would suggest starting very simply - parse rectangles, ellipses, arrows, blocks of text, grouping.  The big differences between VPLs and text-only programming languages include:
	...
	- vector graphics instead of bitmaps (i.e. characters)
		- e.g. with vector graphics, you can resize a rectangle, but it remains recognizable and parsable as a rectangle
...
- Eschew auto layout until we have a syntactic/semantic theory for what layout means
	- experiment, yes 
		...
		- develop a "theory" about what layout means, what information it carries
...
- text-only languages should have 2 syntaxes
	...
	- modern parsing tools (PEG, OhmJS, etc.) make this previously-thought-to-be-decadent-luxury easily realizable

- text should come with a "knob" that can be turned to make the text more dense (e.g. Greek letters) or less dense (long, meaningful phrases)
	- each "variable" should have several names (dense ... less dense) and a separate "knob"

- tooltips
	- tooltips are but an early-stage interface that could be used for enhancing readability of code and phrases
